    ...
    while true do
        ...
        if payAmount > 0 then
            for shop = 1, #Config.Locations do
                ...
                if dist <= 20.0 then
                    if dist >= 12.0 then
                        pNotify("Ai iesit din magazin ti-am golit carucioru !", "error", 2500)
                        payAmount = 0
                        Basket = {}
                    end
                end
...
OpenAction = function(action, shelf, text)
    if action["value"] == "checkout" then
        if payAmount > 0 and #Basket then
            ...
            pNotify("Nu ai nimic in carucior !", 'error', 1500)
        end
    else
...
CashRegister = function(titel)
        local elements = {
            {label = '<span style="color:lightgreen; border-bottom: 1px solid lightgreen;">Confirma</span>', value = "yes"},
            {label = 'Cantitate: <span style="color:green">$' .. payAmount ..'</span>'},
        }
            ...
            function(data, menu)
                ...
                if data.current.value == "yes" then
                    ...
                    ESX.UI.Menu.Open(
                        ...
                        {
                            title    = "Magazin - Mod de plata",
                            align    = 'center',
                            elements = {
                                {label = "Gheata", value = "cash"},
                                {label = "Card", value = "bank"},
                            },
                        },
                        function(data2, menu2)
                            ESX.TriggerServerCallback('99kr-shops:CheckMoney', function(hasMoney)
                                if hasMoney then
                                    ...
                                    pNotify("Nu ai suficienti bani!", 'error', 1500)
                                end
...
ShelfMenu = function(titel, shelf)
    ...
    ESX.UI.Menu.Open(
        ...
        function(data, menu)
            ...
            pNotify("Ai pus " .. data.current.value .. " pretul este de " .. data.current["realLabel"] .. " in cos", 'alert', 1500)           
        end,
    function(data, menu)
        menu.close()
    end)
...
OpenBasket = function()
    if payAmount > 0 and #Basket then
        local elements = {
            {label = 'Cantitate de a plati: <span style="color:green">$' .. payAmount},
        }
        for i=1, #Basket do
            local item = Basket[i]
            table.insert(elements, {
        ...
        ESX.UI.Menu.Open(
            ...
            function(data, menu)
                if data.current.value == 'empty' then
                    ...
                    payAmount = 0
                    menu.close()
                    pNotify("Scote tot din carutu de cumparaturi !", "error", 2500)
                end
                if data.current.value == "item_menu" then
                    ...
                    ESX.UI.Menu.Open(
                        ...
                        {
                            title    = "Carut de cumparaturi - " .. shopItem["label"] .. " - " .. shopItem["amount"] .. "pcs",
                            align    = 'center',
                            elements = {
                        ...
                        function(data2, menu2)
                            if data2.current["value"] == "deleteItem" then
                                pNotify("S-o sters " .. Basket[index]["amount"] .." ".. Basket[index]["label"] .. " din carutu de cumparaturi.", "alert", 2500)
